...
Prefer YAML for hand-maintained workflows and JSON for files generated by Olive. JSON does not allow
comments or trailing commas.

## Start from generated configuration

The safest way to create a workflow for the installed Olive version is to generate one:
...
For a hand-authored starting point, copy `assets/workflow.yaml` from this skill.
The bundled Hugging Face conversion example normally needs `olive-ai[cpu,optimum]` and `transformers`.
...
Only one accelerator is currently supported per system. If systems are omitted, Olive defaults to the
local system and infers available providers where possible.
...
workflow-level values for one pass.
...
unless the user intends to discard cached intermediate results. `no_artifacts: true` skips generated
output artifacts and is not suitable when the user expects a model directory.

## Custom code

Model loaders, data components, custom metrics, and custom evaluators can live in a user script:
...
Use registered function names in the corresponding config fields. Do not embed arbitrary Python code in
YAML or JSON.
