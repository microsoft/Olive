...
def test_mha2sha_pass(tmp_path):
    # setup
    ...
    static_llm_path = tmp_path / "static_llm"
    static_llm = create_pass_from_dict(StaticLLM, {"batch_size": 1, "context_length": 64}, disable_search=True).run(
        split_model, static_llm_path
    )

    output_model_path = tmp_path / "output_model"
    mha2sha_pass = create_pass_from_dict(QairtMHA2SHA, {}, disable_search=True)
    output_model = mha2sha_pass.run(static_llm, output_model_path)
