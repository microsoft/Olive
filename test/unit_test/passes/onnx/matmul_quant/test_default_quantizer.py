...
class TestDefaultWeightOnlyQuantizer:
    ...
    def _assert_quantize_matmul_call_args(self, mock_quantize, packed, fp32weight, scales, zero_point, block_size, cols, rows, is_symmetric):
    ...
    def test_quant_slice_symmetric(self):
        ...
        expected_quantized_slice = np.array([
            [-2, -4,  6, -8],
            [-5,  6,  7, -8]
        ], dtype=np.int8)
