...
from onnx import TensorProto, ModelProto, GraphProto, NodeProto

from olive.passes.onnx.matmul_quant.hqq_quantizer import HQQWeightOnlyQuantConfig, HQQWeightOnlyQuantizer
from olive.passes.onnx.matmul_quant.utils import Algorithm, MSFT_DOMAIN, OpType


@pytest.fixture
def default_config():
    return HQQWeightOnlyQuantConfig(
        block_size=128, 
        bits=4,
        axis=1,
...
class TestHQQWeightOnlyQuantizer:
    ...
    def test_optimize_weights(self):
        ...
        # Execute
        opt_scale, opt_zero = HQQWeightOnlyQuantizer.optimize_weights(tensor, scale, zero, min_max, axis=1, verbose=False)
    ...
    def test_pack_on_row_fast_248bit_4bit(self):
        ...
        for i in range(10):  # Check a few random positions
    ...
    def test_quantize_matmul_node(
        self, 
        mock_get_initializer, 
        mock_quantize_internal, 
        default_config
    ):
        # Setup
        node = MagicMock(spec=NodeProto)
