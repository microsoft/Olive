...
def _make_composite_handler(
    root: Path, components: dict[str, str], model_attributes: dict | None = None
...
class TestCompositeToOnnxPackage:
    ...
    def test_enforces_package_model_attributes(self, tmp_path):
        src_root = _make_nested_genai_package(tmp_path / "src", {"decoder": "decoder/model.onnx"})
        composite = _make_composite_handler(
            src_root,
            {"decoder": "decoder/model.onnx"},
            model_attributes={"ort_genai_package": False, "entry_point_component": "untrusted", "source": "mobius"},
        )

        p = create_pass_from_dict(CompositeToOnnxPackage, {}, disable_search=True)
        out = p.run(composite, str(tmp_path / "out"))

        assert out.model_attributes == {
            "ort_genai_package": True,
            "entry_point_component": "decoder",
            "source": "mobius",
        }
    ...
    def test_rejects_entry_component_outside_package(self, tmp_path):
        src_root = _make_nested_genai_package(tmp_path / "src", {"decoder": "../outside.onnx"})
        composite = _make_composite_handler(src_root, {"decoder": "../outside.onnx"})

        p = create_pass_from_dict(CompositeToOnnxPackage, {}, disable_search=True)
        with pytest.raises(ValueError, match="escapes the ORT-GenAI package directory"):
            p.run(composite, str(tmp_path / "out"))
