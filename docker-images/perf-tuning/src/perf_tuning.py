...
class PerfTestParams:
    ...
    def get_percentiles_args(self, result_file):
        return self.test_args + [
            "-o", self.args.optimization_level, "-m", "times", "-r", "200", self.model, result_file
        ]
        ...
        code_snippet = {
            ...
            "\
                ...
                ".format(self.get_graph_optimization_level(),
                         ...
                         self.get_intra_threads_code_snippet(), self.get_inter_threads_code_snippet(), self.args.model,
...
def run_perf_tuning(test_params, percentiles=False):
    ...
    if perf_tuning.returncode == 0 and os.path.exists(result_file):
        ...
        if percentiles:
            ...
            test_params.memory = round(sum(m.recorded_memory) /
                                       len(m.recorded_memory), 5) if len(m.recorded_memory) > 0 else 0
            if perf_tuning.returncode == 0:
                # Find profile result
                files = glob.glob(profile_name + "*")
                if len(files) == 1:
                    with open(files[0]) as profile:
                        with open(os.path.join(test_params.result_dir, "profile_" + test_params.name + ".tsv"),
                                  "w") as out_profile:
                            print("cat\tpid\ttid\tdur\tts\tph\tname\targs", file=out_profile)
                            for record in json.load(profile):
                                print(str(record["cat"]) + "\t" + str(record["pid"]) + "\t" + str(record["tid"]) +
                                      "\t" + str(record["dur"]) + "\t" + str(record["ts"]) + "\t" + str(record["ph"]) +
                                      "\t" + str(record["name"]) + "\t" + str(record["args"]),
                                      file=out_profile)
...
def run_perf_tuning_binary(test_params,
                           num_cores,
                           name_suffix,
                           desc_suffix,
                           failed_tests,
...
class ConverterParamsFromJson():
    def __init__(self):
        ...
        if loaded_json.get("config") and ["Debug", "MinSizeRel", "Release", "RelWithDebInfo"].index(
                loaded_json["config"]) == -1:
            raise ValueError(
                "Please specify config with one of the following -\"Debug\", \"MinSizeRel\", \"Release\", \"RelWithDebInfo\""
            )
        ...
        self.intra_op_num_threads = loaded_json["intra_op_num_threads"] if loaded_json.get(
            "intra_op_num_threads") else str(cores)
...
def parse_arguments():
    ...
    cores = os.cpu_count()
    print("Cores: ", cores)
    parser.add_argument("--input_json", help="A JSON file specifying the run specs. ")
    ...
    parser.add_argument(
        ...
        default=str(cores),
        help=
        "Used when -P is set. Sets the number of threads used to parallelize the execution of the graph (across nodes), \
                        A value of 0 means ORT will pick a default. Must >=0.")
    parser.add_argument("-s",
                        "--top_n",
                        default="3",
                        help="Show percentiles for top n runs in each execution provider. Default:3.")
    parser.add_argument("-P", "--parallel", default=True, help="Use parallel executor instead of sequential executor.")
    parser.add_argument(
        "-o",
        "--optimization_level",
        default="99",
        help=
        "0: disable optimization, 1: basic optimization, 2: extended optimization, 99: extended+layout optimization.")
...
if __name__ == "__main__":
    ...
    providers = [p for p in args.execution_provider.split(",")
                 if p != ""] if len(args.execution_provider) > 0 else allProviders
