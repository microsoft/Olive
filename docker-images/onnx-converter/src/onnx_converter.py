...
def get_args():
    ...
    parser.add_argument("--input_json", required=False, help="A JSON file specifying the run specs. ")
    ...
    parser.add_argument("--caffe_model_prototxt",
                        required=False,
...
class ConverterParamsFromJson():
    def __init__(self):
        ...
        self.model_outputs_names = loaded_json["model_outputs_names"] if loaded_json.get(
            "model_outputs_names") else None
        self.model_input_shapes = shape_type(
            loaded_json["model_input_shapes"]) if loaded_json.get("model_input_shapes") else None
        ...
        self.caffe_model_prototxt = loaded_json["caffe_model_prototxt"] if loaded_json.get(
            "caffe_model_prototxt") else None
...
def tf2onnx(args):
    if get_extension(args.model) == "pb":
        ...
        subprocess.check_call([
            "python", "-m", "tf2onnx.convert", "--input", args.model, "--output", args.output_onnx_path, "--inputs",
            args.model_inputs_names, "--outputs", args.model_outputs_names, "--opset", args.target_opset,
            "--fold_const", "--target", "rs6"
        ])
    elif get_extension(args.model) == "meta":
        if not args.model_inputs_names and not args.model_outputs_names:
            raise ValueError(
                "Please provide --model_inputs_names and --model_outputs_names to convert Tensorflow checkpoint models."
            )
        subprocess.check_call([
            "python", "-m", "tf2onnx.convert", "--checkpoint", args.model, "--output", args.output_onnx_path,
            "--inputs", args.model_inputs_names, "--outputs", args.model_outputs_names, "--opset", args.target_opset,
            "--fold_const", "--target", "rs6"
        ])
    else:
        subprocess.check_call([
            "python", "-m", "tf2onnx.convert", "--saved-model", args.model, "--output", args.output_onnx_path,
            "--opset", args.target_opset, "--fold_const", "--target", "rs6"
        ])
...
def main():
    ...
    if args.input_json != None and len(args.input_json) > 0:
        ...
        if not args.model_type or len(args.model_type) == 0:
            raise ValueError(
                "Please specify the required argument \"model_type\" either in a json file or by --model_type")
        if not args.output_onnx_path or len(args.output_onnx_path) == 0:
            raise ValueError(
                "Please specify the required argument \"output_onnx_path\" either in a json file or by --ouptut_onnx_path"
            )
    ...
    # Test correctness
    verify_status = check_model(args.model, args.output_onnx_path, inputs_path, args.model_type,
                                args.model_inputs_names, args.model_outputs_names)
