...
def setup():
    ...
    example_dir = cur_dir / "resnet_vitis_ai_ptq_cpu"
...
def check_output(footprints):
    for footprint in footprints.values():
        ...
        for v in footprint.nodes.values():
            assert all([value.value > 0 for value in v.metrics.value.values()])
...
@pytest.mark.parametrize("olive_json", ["resnet_static_config.json"])
