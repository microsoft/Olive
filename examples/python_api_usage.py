...
def example_run_config():
    ...
    # Run the workflow
    workflow_output = run(config=config, output_path="./config_output")
...
def example_utility_functions():
    ...
    from olive import convert_adapters, extract_adapters

    # Convert LoRA adapters to ONNX format (no return value)
    convert_adapters(
        adapter_path="./fine_tuned_adapter", output_path="./converted_adapter.onnx", adapter_format="onnx_adapter"
    )

    # Extract LoRA adapters from PyTorch model (no return value)
    extract_adapters(model_path="./pytorch_model_with_lora", output_path="./extracted_adapters", format="onnx_adapter")
