...
def export_onnx(model_name, model_root_path, device="cpu"):
    ...
    main_export(model_name, onnx_model_path) if device == "cpu" else main_export(model_name, onnx_model_path, device="cuda")
...
def run_bert_inc(cur_dir, metric_res, test_num):
    ...
    for _ in range(test_num):
        ...
        with open(olive_config_path, "r") as fin:
            ...
            olive_config["evaluators"]["common_evaluator"]["metrics"][0]["user_config"]["user_script"] = user_script_path
            olive_config["evaluators"]["common_evaluator"]["metrics"][1]["user_config"]["user_script"] = user_script_path
        ...
        for optimized_model in model_list:
            ...
            with open(config_json_path, "r") as fin:
                ...
                if optimized_model == "onnx":
                    olive_config["input_model"]["config"]["model_path"] = str(Path(model_root_path / optimized_model / "model.onnx"))
                elif optimized_model == "optimum_dynamic_quantization":
                    olive_config["input_model"]["config"]["model_path"] = str(Path(model_root_path / optimized_model / "model_quantized.onnx"))
                elif optimized_model in ["optimum_o1", "optimum_o2", "optimum_o3", "optimum_o4"]:
                    olive_config["input_model"]["config"]["model_path"] = str(Path(model_root_path / optimized_model / "model_optimized.onnx"))
                ...
                olive_config["engine"]["execution_providers"] = ["CPUExecutionProvider"] if device == "cpu" else ["CUDAExecutionProvider"]
