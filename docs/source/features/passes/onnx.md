...
Users that write their own pre/post processing steps need to have the knowlege about whether the step includes the operators that is built-in support or supported in onnxextension.
For example, for some ops like `ConvertImageToBGR` which require other extensions may be incompatible with ort-web, user need to exclude this kind of ops to generate proper models.
