...
* `PackagingConfig`
    ...
    * `extra_config [dict]`:
      This extra_config is for other configuration that might be added in the future. Currently it has:
        * `export_model_in_mlflow_format [bool]`: Whether user wants to export output ONNX model in mlflow model format.
