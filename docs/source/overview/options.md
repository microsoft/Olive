...
- `config: [Dict]` For example, for `PytorchModel`, the input model config dictionary specifies following items:
    ...
    model. This dictionary specifies the following items:
        ...
        - `model_loading_args: [dict]`: Arguments to pass to the `from_pretrained` method of the model class. Refer to [this documentation](https://huggingface.co/docs/transformers/main_classes/model#transformers.PreTrainedModel.from_pretrained).
