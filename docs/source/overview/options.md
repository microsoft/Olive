...
- `config: [Dict]` The input model config dictionary specifies following items:
    ...
    model. This dictionary specifies the following items:
        ...
        - `dataset: [dict]`: If you want to use the huggingface dataset, you need to provide the dataset config. See [huggingface datasets](https://huggingface.co/docs/datasets/loading). Olive exposes the following configs(which will be extend in the future):
            ```python
            "dataset": {
