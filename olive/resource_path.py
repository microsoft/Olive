...
class ResourcePath(ConfigBase):
    ...
    @classmethod
    def create_resource_path(cls, resource_path: Union[str, Path, Dict[str, Any], "ResourcePath"]) -> "ResourcePath":
        """
        ...
        """
        if isinstance(resource_path, ConfigBase):
            return validate_config(resource_path, cls)
    ...
    def save_to_dir(self, dir_path: Union[Path, str], name: str = None, overwrite: bool = False) -> str:
        """Save the resource to a directory."""
        return self.config.save_to_dir(dir_path, name, overwrite)
...
class LocalResourceConfig(ResourceConfig):
    ...
    def save_to_dir(self, dir_path: Union[Path, str], name: str = None, overwrite: bool = False) -> str:
        ...
        if name:
            new_path_name = Path(name).with_suffix(self.path.suffix).name
        else:
            new_path_name = self.path.name
...
class AzureMLDatastoreConfig(ResourceConfig):
    ...
    def get_path(self) -> str:
        workspace_config = self.azureml_client.get_workspace_config()
        return (
    ...
    def save_to_dir(self, dir_path: Union[Path, str], name: str = None, overwrite: bool = False) -> str:
        ...
        azureml_model_resource = AzureMLModelConfig(
            azureml_client=self.azureml_client, name=aml_model.name, version=aml_model.version
        )
...
class AzureMLJobOutputConfig(ResourceConfig):
    ...
    def save_to_dir(self, dir_path: Union[Path, str], name: str = None, overwrite: bool = False) -> str:
        ...
        # path to save the resource to
        if name:
            new_path_name = Path(name).with_suffix(Path(self.relative_path).suffix).name
        else:
            new_path_name = Path(self.relative_path).name
        new_path = dir_path / new_path_name
        _overwrite_helper(new_path, overwrite)
