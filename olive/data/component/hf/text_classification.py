...
from olive.data.component.hf.utils import TokenizerConfig, map_pre_process, truncate_dataset


class CommonClassificationParams(TokenizerConfig):
    """Common parameters for text classification and ner tasks"""
...
class TextClassificationParams(CommonClassificationParams):
...
def text_classification_pre_process(dataset, model_name, **kwargs):
    ...
    tokenized_dataset = map_pre_process(dataset, tokenize_and_add_labels, batched=True, remove_cols=True)
    # label_cols is ["label"] since we added label_cols[0] as "label" to tokenized_inputs
    return BaseDataset(tokenized_dataset, label_cols=["label"], max_samples=args.max_samples)


def ner_pre_process(dataset, model_name, **kwargs):
    """Pre-process data for ner task."""

    args = validate_config(kwargs, CommonClassificationParams, warn_unused_keys=True)
    ...
    def tokenize_and_align_labels(examples):
        tokenized_examples = tokenizer(examples, **tokenization_kwargs)
        all_labels = examples[args.label_cols[0]]
        new_labels = []
        for i, labels in enumerate(all_labels):
            word_ids = tokenized_examples.word_ids(i)
            new_labels.append(align_labels_with_tokens(labels, word_ids))

        tokenized_examples["label"] = new_labels
        return tokenized_examples

    tokenized_dataset = map_pre_process(dataset, tokenize_and_align_labels, batched=True, remove_cols=True)
    # label_cols is ["label"] since we added label_cols[0] as "label" to tokenized_inputs
    return BaseDataset(tokenized_dataset, label_cols=["label"], max_samples=args.max_samples)
