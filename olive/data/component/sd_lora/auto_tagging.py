...
# --------------------------------------------------------------------------
"""Auto tagging components for Stable Diffusion LoRA training."""
...
def _save_tags(image_path: str, tags: list[str], tag_extension: str = ".txt", separator: str = ", ") -> None:
    """Save tags to a text file alongside the image."""
    tag_path = Path(image_path).with_suffix(tag_extension)
    tag_path.write_text(separator.join(tags), encoding="utf-8")
...
def auto_tagging(
    ...
    **kwargs,
    ...
    """Auto-generate tags for images using tagger models.

    Supports multiple tagging backends:
    - wd14: WaifuDiffusion tagger (v14, swinv2, convnext, etc.)
    - deepdanbooru: DeepDanbooru tagger

    Args:
        dataset: The dataset to process.
        model_type: Type of tagging model ("wd14", "deepdanbooru").
        model_name: Specific model name. If None, uses default for model_type.
        threshold: Confidence threshold for tags.
        general_threshold: Threshold for general tags (if None, uses threshold).
        character_threshold: Threshold for character tags (if None, uses threshold).
        ...
        include_rating: Whether to include rating tags (safe, questionable, explicit).
        exclude_tags: List of tags to exclude from results.
        replace_underscore: Whether to replace underscores with spaces in tags.
        max_tags: Maximum number of tags to include (by confidence).
    ...
    if model_type == "wd14":
        ...
        raise ValueError(f"Unsupported model type: {model_type}. Supported: wd14")
...
def wd14_tagging(
    ...
    **kwargs,
    ...
    - SmilingWolf/wd-swinv2-tagger-v3 (recommended, most accurate)
    ...
    for i in range(0, len(dataset), batch_size):
        ...
        for item in batch_items:
            tag_path = Path(item["image_path"]).with_suffix(tag_extension)
        ...
        # Process each result
        for idx, (item, probs) in enumerate(zip(items_to_process, outputs)):
            tags_result = []

            # Process general tags
            for tag_idx, tag_name in general_tags:
                if probs[tag_idx] >= general_threshold:
            ...
            for tag_idx, tag_name in character_tags:
                if probs[tag_idx] >= character_threshold:
                    if tag_name not in exclude_tags:
                        if replace_underscore:
                            tag_name = tag_name.replace("_", " ")
                        tags_result.append((tag_name, probs[tag_idx]))
            ...
            # Save tags
            _save_tags(item["image_path"], final_tags, tag_extension, tag_separator)
            processed_count += 1
