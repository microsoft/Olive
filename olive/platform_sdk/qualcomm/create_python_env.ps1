...
& (Join-Path $FILES_DIR $PY_ENV_NAME "python.exe") -m pip install --upgrade pip
if ($PY_VERSION -eq "3.6") {
    & (Join-Path $FILES_DIR $PY_ENV_NAME "python.exe") -m pip install onnx==1.11.0 onnx-simplifier packaging tensorflow==1.15.0 pyyaml
}
elseif ($PY_VERSION -eq "3.8") {
    & (Join-Path $FILES_DIR $PY_ENV_NAME "python.exe") -m pip install onnx onnx-simplifier packaging tensorflow pyyaml
}
else {
