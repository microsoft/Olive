...
def configure_dev(py_version: str):
    ...
    logger.info("Configuring %s with python %s...", sdk_arch, py_version)
    cmd = None
    with resources.path(resource_path, script_name) as create_python_env_path:  # pylint: disable=deprecated-method
        if platform.system() == OS.LINUX:
            cmd = f"bash {create_python_env_path} -v {py_version}"
        elif platform.system() == OS.WINDOWS:
            cmd = f"powershell {create_python_env_path} {py_version}"
...
def configure_eval():
    ...
    logger.info("Configuring for %s...", target_arch_name)
