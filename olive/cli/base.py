...
def get_hf_input_model(args, model_path):
    input_model = {
        "type": "HfModel",
...
def get_onnx_input_model(model_path):
    return {
        "type": "OnnxModel",
        "model_path": model_path,
    }
...
def get_pt_input_model(args, model_path):
...
def get_input_model_config(args) -> Union[str, Dict[str, str]]:
    ...
    if model_name_or_path is None:
        ...
        return get_pt_input_model(args, None)
    ...
    if match:
        return get_hf_input_model(args, match.group(1))

    # Check AzureML model
    pattern = r"^azureml:(?P<model_name>[^:]+):(?P<version>[^:]+)$"
    match = re.match(pattern, model_name_or_path)
    if match:
        return get_pt_input_model(
            args,
            {
                "type": "azureml_model",
                "name": match.group("model_name"),
    ...
    if not model_path.exists():
        return get_hf_input_model(args, model_name_or_path)
    ...
    if model_path.is_file():
        if model_path.suffix in (".pt", ".pth"):
            return get_pt_input_model(args, model_name_or_path)
        if model_path.suffix == ".onnx":
            return get_onnx_input_model(model_name_or_path)
