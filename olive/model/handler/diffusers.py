...
# --------------------------------------------------------------------------
"""Diffusers model handler for Stable Diffusion, SDXL, Flux and other diffusion models."""
...
class DiffusersModelHandler(OliveModelHandler):
    ...
    Unlike HfModelHandler, it doesn't require a model_type in config.json.

    Example usage:
        model = DiffusersModelHandler(
            model_path="runwayml/stable-diffusion-v1-5",
            model_type_hint="sd15",  # optional: sd15, sdxl, flux, or auto
        )
    """

    resource_keys: tuple[str, ...] = ("model_path", "adapter_path")
    json_config_keys: tuple[str, ...] = ("model_type_hint", "load_kwargs")

    def __init__(
        self,
        model_path: OLIVE_RESOURCE_ANNOTATIONS,
        model_type_hint: str = "auto",
        ...
        Args:
            ...
            model_type_hint: Hint for model type detection: 'sd15', 'sdxl', 'flux', or 'auto'.
        ...
        self.model_type_hint = model_type_hint
        self.load_kwargs = load_kwargs or {}
        self._pipeline = None

        # Detect and store model type
        self._detected_model_type = None
    ...
    @property
    def detected_model_type(self) -> str:
        """Detect and return the model type (sd15, sdxl, flux)."""
        if self._detected_model_type is None:
            self._detected_model_type = self._detect_model_type()
        return self._detected_model_type

    def _detect_model_type(self) -> str:
        """Detect the diffusion model type."""
        if self.model_type_hint != "auto":
            return self.model_type_hint.lower()
