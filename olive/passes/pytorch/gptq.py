...
class GptqQuantizer(Pass):
    ...
    def _run_for_config(
        ...
        for key in ["outside_layer_modules", "inside_layer_modules", "layers_block_name"]:
            if config[key]:
                ...
                setattr(quantized_model, key, self.get_gptq_info(model_adapter, key))
            else:
    ...
    @staticmethod
    def get_gptq_info(model_adapter: ModelAdapter, name: str) -> List[str]:
        """Get the GPTQ info from the model adapter."""
        if name == "outside_layer_modules":
            return [*model_adapter.get_embeds()[1], model_adapter.get_pre_head_layernorm()[1]]
        if name == "inside_layer_modules":
            layer_adapter = model_adapter.get_layer_adapters()[0]
            return [
                layer_adapter.get_attention_inputs()[1],
                layer_adapter.get_attention_outputs()[1],
                layer_adapter.get_mlp_inputs()[1],
                layer_adapter.get_mlp_outputs()[1],
            ]
        if name == "layers_block_name":
            return model_adapter.get_layers()[1]
