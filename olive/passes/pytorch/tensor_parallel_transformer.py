...
class TransformerTensorParallel(TensorParallel):
    ...
    def _apply_transformer(self, model: torch.nn.Module) -> None:
        if (model.config.n_head % self.world_size) != 0:
            raise ValueError(
                f"World size (world_size={self.world_size}) should be a multiple "
                f"of number of heads in the model (nheads={model.config.n_head})"
            )

        if (model.config.dim % self.world_size) != 0:
            raise ValueError(
                f"World size (world_size={self.world_size}) should be a multiple "
                f"of hidden size in the model (dim={model.config.dim})"
            )
        ...
        model.config.n_local_head = model.config.n_local_head // self.world_size
    ...
    def _apply_attention(self, attn: torch.nn.Module) -> None:
        ...
        kv_size = attn.n_local_head * attn.head_dim
        ...
        attn.n_local_head = attn.n_local_head // self.world_size
