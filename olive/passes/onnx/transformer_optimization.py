...
class OrtTransformersOptimization(Pass):
    ...
    def _run_for_config(
        ...
        if model.model_attributes:
            model_adapter = ModelAdapter(model.model_attributes)

            model_type = MODEL_TYPE_MAPPING.get(model_adapter.model_type, model_adapter.model_type)
            ...
            if run_config["num_heads"] == 0 and model_adapter.num_attention_heads:
                run_config["num_heads"] = model_adapter.num_attention_heads
                logger.debug("num_heads is set to %d from model attributes", run_config["num_heads"])
            if run_config["hidden_size"] == 0 and model_adapter.hidden_size:
                run_config["hidden_size"] = model_adapter.hidden_size
                logger.debug("hidden_size is set to %d from model attributes", run_config["hidden_size"])
            if num_kv_heads == 0 and model_adapter.num_key_value_heads:
                num_kv_heads = model_adapter.num_key_value_heads
