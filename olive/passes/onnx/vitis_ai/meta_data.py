...
from olive.passes.onnx.common import process_llm_pipeline, resave_model,model_proto_to_olive_model
...
class VitisAIAddMetaData(Pass):
    ...
    def _run_for_config(
        ...
        if isinstance(model, ONNXModelHandler):
            ...
            return model_proto_to_olive_model(onnx_model, output_model_path, config) # used in transformer optimizations, we can pass model save options in config 
            ...
            model, pipeline, process_context_iterator, output_model_path
        )
