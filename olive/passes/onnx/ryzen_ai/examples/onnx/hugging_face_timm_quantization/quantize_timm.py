...
def prepare_calib_and_eval_data(data_path: str) -> Tuple[str, str]:
    ...
    if not os.listdir(evaluation_data_path):
        print("The extraction will take a few minutes.")
        result = subprocess.run(["tar", "-xzf", data_path, "-C", "val_data"], capture_output=True, text=True)
        print(result.stdout)
    if not os.listdir(calibration_data_path):
        ...
        destination_folder = calibration_data_path

        subfolders = os.listdir(evaluation_data_path)

        for subfolder in subfolders[:100]:
            source_subfolder = os.path.join(evaluation_data_path, subfolder)
    ...
    return calibration_data_path, evaluation_data_path
...
def main(args: argparse.Namespace) -> None:
    ...
    if not args.data_path and not (args.calib_data_path and args.eval_data_path):
        print("Please provide (data_path) or (calib_data_path and eval_data_path).")
    if args.data_path:
