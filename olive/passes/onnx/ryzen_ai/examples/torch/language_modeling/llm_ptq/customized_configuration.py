...
from quark.torch.quantization import QuantizationConfig, FP8E4M3PerTensorSpec, \
    ...
    Uint8PerTensorSpec, Int8PerGroupSpec, Uint8PerGroupSpec, BFP16Spec, MX6Spec, Float16Spec, Bfloat16Spec, \
    FP8E4M3PerChannelSpec, Uint4PerChannelSpec, FP4PerGroupSpec, FP8E4M3PerGroupSpec, FP8E5M2PerGroupSpec, OCP_MXFP4DiffsSpec, \
    FP6E2M3PerGroupSpec, FP6E3M2PerGroupSpec, ScaleQuantSpec, OCP_MXFP4Spec, OCP_MXFP6E2M3Spec, OCP_MXFP6E3M2Spec, OCP_MXFP8E4M3Spec, OCP_MXFP8E5M2Spec, OCP_MXINT8Spec, Int2PerGroupSpec
...
] + OCP_MX_QUANT_SCHEME + list(DEPRECATED_QUANT_SCHEME.keys())
...
OCP_MXFP8_STATIC_SPEC = OCP_MXFP8E4M3Spec(ch_axis=-1,
                                          is_dynamic=False).to_quantization_spec()

OCP_MXFP8_DYNAMIC_SPEC = OCP_MXFP8E4M3Spec(ch_axis=-1,
                                           is_dynamic=True).to_quantization_spec()
...
FP4_PER_GROUP_SYM_STATIC_GS16_SPEC = FP4PerGroupSpec(
                                             ...
                                             is_dynamic=False,
                                             scale_format="e8m0",
                                             scale_calculation_mode="even").to_quantization_spec()

FP4_PER_GROUP_SYM_DYNAMIC_GS16_SPEC = FP4PerGroupSpec(ch_axis=-1,
                                                      group_size=16,
                                                      is_dynamic=True,
                                                      scale_format="e8m0",
                                                      scale_calculation_mode="even").to_quantization_spec()

OCP_MX_SEPERATED_FP4_PER_GROUP_DIFFS_SPEC = OCP_MXFP4DiffsSpec(ch_axis=-1,
                                                               is_dynamic=False).to_quantization_spec()
...
def FP8_E5M2_PER_GROUP_SYM_SPEC(group_size, scale_format="e8m0", scale_calculation_mode="even", is_dynamic=True):
    return FP8E5M2PerGroupSpec(ch_axis=-1,
                               group_size=group_size,
                               scale_format=scale_format,
                               scale_calculation_mode=scale_calculation_mode,
                               is_dynamic=is_dynamic).to_quantization_spec()
...
def get_global_config(quant_scheme: str, group_size: int, scale_format: str = 'e4m3', scale_calculation_mode: str = 'even') -> QuantizationConfig:
    if quant_scheme not in SUPPORTED_QUANT_SCHEME:
        raise ValueError(f"The quant_scheme {quant_scheme} is not supported, only {SUPPORTED_QUANT_SCHEME} are supported.")

    if quant_scheme in OCP_MX_QUANT_SCHEME:
        if group_size != 32:
            raise ValueError(f"The quant_scheme {quant_scheme} requires group_size=32, got group_size={group_size}. Please use the option `--group-size 32`.")
    ...
    if quant_scheme in QUANT_SCHEME_TO_CONFIG:
        ...
        global_quant_config = QuantizationConfig(input_tensors=FP8_PER_TENSOR_SPEC, weight=OCP_MXFP4_SPEC(scale_calculation_mode, False))
    elif quant_scheme == 'w_mxfp4_a_mxfp4':
        global_quant_config = QuantizationConfig(input_tensors=FP4_PER_GROUP_SYM_SPEC(group_size, "e8m0", scale_calculation_mode, True), weight=FP4_PER_GROUP_SYM_SPEC(group_size, "e8m0", scale_calculation_mode, False))
    elif quant_scheme == 'w_fp4_a_fp4_dynamic_per_group_sym_gs16_scale_e8m0':
        global_quant_config = QuantizationConfig(input_tensors=FP4_PER_GROUP_SYM_DYNAMIC_GS16_SPEC, weight=FP4_PER_GROUP_SYM_STATIC_GS16_SPEC)
    elif quant_scheme == 'w_mxfp4_a_mxfp6':
        global_quant_config = QuantizationConfig(input_tensors=OCP_MXFP6_E2M3_SPEC(group_size, scale_calculation_mode, True), weight=OCP_MXFP4_SPEC(group_size, scale_calculation_mode, False))
        ...
        global_quant_config = QuantizationConfig(input_tensors=OCP_MXFP4_SPEC(scale_calculation_mode, True), weight=OCP_MXFP4_SPEC(scale_calculation_mode, False))
        ...
        global_quant_config = QuantizationConfig(input_tensors=OCP_MXFP6_E2M3_SPEC(group_size, scale_calculation_mode, True), weight=OCP_MXFP6_E2M3_SPEC(group_size, scale_calculation_mode, False))
    elif quant_scheme == 'w_mxfp6_e3m2_a_mxfp6_e3m2':
        global_quant_config = QuantizationConfig(input_tensors=OCP_MXFP6_E3M2_SPEC(group_size, scale_calculation_mode, True), weight=OCP_MXFP6_E3M2_SPEC(group_size, scale_calculation_mode, False))
    elif quant_scheme == 'w_fp6_e2m3_per_group_a_fp6_e2m3_per_group':
        global_quant_config = QuantizationConfig(input_tensors=FP6_E2M3_PER_GROUP_SYM_SPEC(group_size, scale_format, None, True), weight=FP6_E2M3_PER_GROUP_SYM_SPEC(group_size, scale_format, None, False))
    elif quant_scheme == 'w_fp6_e3m2_per_group_a_fp6_e3m2_per_group':
        global_quant_config = QuantizationConfig(input_tensors=FP6_E3M2_PER_GROUP_SYM_SPEC(group_size, scale_format, None, True), weight=FP6_E3M2_PER_GROUP_SYM_SPEC(group_size, scale_format, None, False))
