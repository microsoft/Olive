    ...
    def _run_for_config(
        ...
        try:
            ...
            if split_assignments or layer_annotations:
                if output_model_filepath.exists():
                    ...
                    logger.warning(
                        "Skipping split_assignments/layer_annotations because expected ONNX file %s does not exist.",
                        output_model_filepath,
                    )
        except Exception:
        ...
        if metadata_only:
            ...
            primary_model_paths = (
                {str(fp) for fp in generated_onnx_files} if is_multi_file_model else {str(output_model_filepath)}
            )
        ...
        if metadata_only:
            ...
            if not output_model_filepath.exists():
                if len(generated_onnx_files) == 1:
                    logger.info(
                        "ONNX model file %s does not exist, using %s instead",
                        output_model_filepath,
                        generated_onnx_files[0].name,
                    )
                    onnx_file_name = generated_onnx_files[0].name
