...
class ModelBuilder(Pass):
    ...
    def _run_for_config(
        ...
        from olive.passes.onnx.model_builder_utils import create_model
        ...
        # TODO(anyone): reconsider this once we investigate symlinks for such cases
        output_model_filepath = (
            Path(resolve_onnx_path(output_model_path)) if not metadata_only else Path(model.model_path)
        )
        extra_args = {"filename": output_model_filepath.name}
        if metadata_only:
            extra_args["config_only"] = True
            model_path = None
            input_path = output_model_filepath.parent
            adapter_path = None
        else:
            model_path = str(model.model_path or model.hf_config.model_name)
            input_path = ""
        ...
        genai_config_filepath = output_model_filepath.parent / "genai_config.json"
        ...
        # add lora_modules to model_attributes if not metadata_only
        if not metadata_only and model.adapter_path:
            from peft import LoraConfig

            lora_config = LoraConfig.from_pretrained(model.adapter_path)
            model_attributes["lora_modules"] = list(lora_config.target_modules)
        ...
        # .onnx and .onnx.data are already accounted for
        filepaths_to_ignore = {str(output_model_filepath), str(output_model_filepath) + ".data"}
        new_files = {
            str(fp.relative_to(output_model_filepath.parent))
            for fp in output_model_filepath.parent.iterdir()
            if str(fp) not in filepaths_to_ignore
        } - set(additional_files)

        additional_files.extend(new_files)
