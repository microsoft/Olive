...
class CompositeToOnnxPackage(Pass):
    ...
    * ORT-GenAI evaluators (``LMMSEvaluator``, ``OnnxEvaluator._inference_vision_genai``)
    ...
    def _run_for_config(
        ...
        dst_dir = self._resolve_output_dir(output_model_path).resolve()
        ...
        entry_path = (dst_dir / entry_filename).resolve()
        if not entry_path.is_relative_to(dst_dir):
            raise ValueError(
                f"Entry-point component '{entry_filename}' escapes the ORT-GenAI package directory {dst_dir}."
            )
