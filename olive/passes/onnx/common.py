...
def _ir_shape_is_fixed(value: Optional[ir.Value]) -> bool:
...
def _iter_graphs_ir(graph: ir.Graph):
    """Yield the graph and all of its subgraphs recursively (for control-flow nodes)."""
    yield graph
    for node in graph:
        for attr in node.attributes.values():
            ...
            if attr.type == ir.AttributeType.GRAPH:
                yield from _iter_graphs_ir(attr.value)
            elif attr.type == ir.AttributeType.GRAPHS:
                for subgraph in attr.value:
                    yield from _iter_graphs_ir(subgraph)


def _iter_shaped_values_ir(graph: ir.Graph):
...
def _make_dim_param_fixed_ir(graph: ir.Graph, param_name: str, value: int) -> None:
    """Replace every occurrence of the symbolic dim ``param_name`` with ``value`` across the graph.

    Mirrors onnxruntime.tools.onnx_model_utils.make_dim_param_fixed but operates on an ir.Graph,
    including subgraphs.
    """
    for subgraph in _iter_graphs_ir(graph):
        for val in _iter_shaped_values_ir(subgraph):
...
def _remove_invalid_dim_values_ir(graph: ir.Graph) -> None:
...
def _make_input_shape_fixed_ir(graph: ir.Graph, input_name: str, fixed_shape: list[int]) -> None:
    """Set the shape of the named graph input to ``fixed_shape``.

    Mirrors onnxruntime.tools.onnx_model_utils.make_input_shape_fixed but operates on an ir.Graph.
    """
    # remove any invalid dim values first. typically this is a dim_value of -1.
    _remove_invalid_dim_values_ir(graph)
    ...
    for graph_input in graph.inputs:
        ...
        for idx, dim in enumerate(dims):
            if isinstance(dim, int):
                ...
                _make_dim_param_fixed_ir(graph, dim.value, fixed_shape[idx])
    ...
    valid_names = ",".join(i.name for i in graph.inputs if i.name)
    raise ValueError(f"Input {input_name} was not found in graph inputs. Valid input names are: {valid_names}")


def _fix_output_shapes_ir(ir_model: ir.Model) -> None:
    ...
    for output in ir_model.graph.outputs:
        if output is None or output.name is None or _ir_shape_is_fixed(output):
            continue
...
def fix_dim_params_ir(ir_model: ir.Model, dim_params: list[str], dim_values: list[int]) -> None:
    ...
    for param, value in zip(dim_params, dim_values):
        _make_dim_param_fixed_ir(ir_model.graph, param, value)

    # update the output shapes to make them fixed
    _fix_output_shapes_ir(ir_model)


def fix_input_shapes_ir(ir_model: ir.Model, input_names: list[str], input_shapes: list[list[int]]) -> None:
    ...
    for name, shape in zip(input_names, input_shapes):
        _make_input_shape_fixed_ir(ir_model.graph, name, shape)
