...
class OnnxConversion(Pass):
    ...
    def _export_pytorch_model(
        ...
        has_adapters = is_peft_model(pytorch_model)
        if has_adapters:
        ...
        if config["use_dynamo_exporter"]:
            ...
            # lora adapters weights are sometimes stored in identity nodes which are not needed
            if has_adapters:
                logger.debug("Removing unnecessary identity nodes from the model.")
                # TODO(jambayk): Use our own onnx dag utils when added to olive
                from onnxruntime.transformers.fusion_utils import FusionUtils
                from onnxruntime.transformers.onnx_model import OnnxModel

                fusion_utils = FusionUtils(OnnxModel(onnx_model))
                fusion_utils.remove_identity_nodes()
        ...
        if not model.is_model_loaded_from_hf_config() or not model.hf_config.from_pretrained_args:
            ...
            if torch_dtype and torch_dtype != model_dtype:
                ...
                logger.warning(
                    ...
                    " use_device as 'cuda' or use OrtTransformerOptimization/OnnxFloatToFloat16 pass after conversion"
                    " to convert the model to float16."
                )
