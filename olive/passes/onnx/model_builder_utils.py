...
class Model:
    ...
    def make_model(self, hf_name, input_path, adapter_path):
        ...
        if input_path.endswith(".gguf"):
            ...
            # Load PyTorch model
            extra_kwargs = {"trust_remote_code": True} if os.path.exists(hf_name) else {"num_hidden_layers": self.num_layers, "trust_remote_code": True} if "num_hidden_layers" in self.extra_options else {"cache_dir": self.cache_dir, "token": True, "trust_remote_code": True}
            model = AutoModelForCausalLM.from_pretrained(hf_name, **extra_kwargs)
...
def create_model(model_name, input_path, adapter_path, output_dir, precision, execution_provider, cache_dir, **extra_options):
    ...
    # Load model config
    extra_kwargs = {"trust_remote_code": True} if os.path.isdir(input_path) else {"cache_dir": cache_dir, "token": True, "trust_remote_code": True}
    ...
    parser.add_argument(
        ...
        help=textwrap.dedent("""\
            Key value pairs for various options. Currently supports:
                ...
                enable_GQA_on_CPU = Enable G(Group)Query(Q)Attention(A) on CPU.
