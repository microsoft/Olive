...
from olive.model.handler.mixin.onnx_graph import get_initializer
...
class OnnxHqqQuantization(Pass):
    ...
    def _run_for_config(
        ...
        model.set_opset_import(MSFT_DOMAIN, 1)
        dag = OnnxDAG(model.load_model())
        dag = self._process_with_dag(dag, config.block_size, config.axis, config.nodes_to_exclude, config.nodes_to_include)
        for node in dag.model.graph.node:
            if node.op_type == str(OpType.MatMulNBits):
                logger.info("MatMulNBits node found in the model.")
                break
        else:
            logger.info("MatMulNBits node not found in the model.")
        ...
        for node in dag.model.graph.node:
            ...
            logger.info("MatMulNBits node not found in the model.")
        dag.update()
        # check if dag.model has matmulnbits node
        for node in dag.model.graph.node:
            if node.op_type == str(OpType.MatMulNBits):
                logger.info("MatMulNBits node found in the model.")
                break
    ...
    def _process_with_dag(
            self, 
            dag: OnnxDAG, 
            block_size: int, 
            axis: int, 
            nodes_to_exclude: List[str], 
            nodes_to_include: List[str]
        ):
        """使用OnnxDAG处理图形并进行量化。
        ...
        Args:
            ...
            block_size: 量化的块大小
            axis: 量化轴
            nodes_to_exclude: 要排除的节点名称列表
            nodes_to_include: 要包含的节点名称列表
        """
        ...
        # 直接修改图形，不尝试更新DAG内部结构
        # DAG.update()会在操作结束后统一处理图形和内部结构的同步
            ...
            if node.proto.name in nodes_to_exclude:
                ...
                if len(quantized_nodes) == 1 and quantized_nodes[0].op_type == str(OpType.MatMulNBits):
                    ...
                    for consumer in dag.get_consumers(node_name):
                        print(f"replace {consumer} {node.proto.output[0]} with {quantized_nodes[0].output[0]}")
