...
class StaticLLM(Pass):
    ...
    def _run_qnn_gpu(self, model: ONNXModelHandler, config: type[BasePassConfig], output_model_path: Path):
        ...
        - If config.weight_sharing is false: generate single model.
        - If config.weight_sharing is true: generate multiple models (ar1 and arN) and return CompositeModelHandler.
        """
        ...
        weight_sharing = getattr(config, "weight_sharing", False)
