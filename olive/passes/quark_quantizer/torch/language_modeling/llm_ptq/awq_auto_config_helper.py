...
class EasyGraph:
    ...
    def _get_connect_info(self, gm):
        def get_next_module(node_name, res):
            can_skip_class_list = {torch.nn.modules.activation.ReLU}
            ...
            for sub_node in self._connect_info[node_name]["children_nodes"]:
                ...
                if (
                    self._connect_info[sub_node.name]["belong_module"] in self._connect_info[node_name]["belong_module"]
                    or self._connect_info[sub_node.name]["class"] in can_skip_class_list
                ):
                    # 1.for case custom nn.module, has inner functional node and nn.module node
                    # 2.some nn.module skip,this is not safe
                    if not all(
                        [name not in str(self._connect_info[sub_node.name]["class"]) for name in can_skip_node_list]
                    ):
        ...
        for node in gm.graph.nodes:
            ...
            self._connect_info[node.name] = {
                "node": node,
                "children_nodes": [node for node in node.users.keys()],
                "op": node.op,
    ...
    def find_pair(self, pair_list):
        def _check_type(test_module, target):
            ...
            if isinstance(target, ModelTypeList):
                return not all(
                    [test_module.__class__.__name__ != model_class_name for model_class_name in target.model_list]
                )
    ...
    def combine_into_json(self, ln_fcs, fc_fc):
        ...
        config_dict["model_decoder_layers"] = (
            "" if prefix_count_dict == {} else max(prefix_count_dict, key=prefix_count_dict.get)
        )
        config_dict["scaling_layers"] = []
        for k, v in pair_dict.items():
            config_dict["scaling_layers"].append(
                {"prev_op": k, "layers": v, "inp": v[0], "module2inspect": self.get_nearest_parent_module(v)}
            )
