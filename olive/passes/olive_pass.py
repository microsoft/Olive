    ...
    def generate_search_space(
        ...
        if not disable_search:
            for name, param in default_config.items():
                if param.search_defaults:
                    value = config[name]

                    # Creat a dummy config class to validate the value
                    dummy_value_config = {name: (param.type_, None)}
                    dummy_value_model = create_model(
                        f"SearchableParamConfig_{name}_value", **dummy_value_config, __base__=BaseModel
                    )

                    # Validate if the user provided a "non-searchable" value
                    # i.e. the "type of the value" provided by the user is the same as the "param's expected type"
                    try:
                        validate_config({name: value}, dummy_value_model)
                        continue
                    ...
                    # If neither scenario suceeds, leave the value alone so that the default validation
