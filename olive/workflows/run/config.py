...
from olive.systems.system_config import AML_PYTHON_ENV_PATH, SystemConfig
...
class RunConfig(NestedConfig):
    ...
    def validate_workflow_host(cls, v, values):
        ...
        for system_name, system_config in systems.items():
            ...
            # For remote workflow, python environment will be installed in the AML_PYTHON_ENV_PATH folder
            # Preset the python_environment_path as a config to avoid the validation error
            system_config.config.python_environment_path = str(Path(AML_PYTHON_ENV_PATH) / system_name / "bin")
