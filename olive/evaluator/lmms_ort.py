...
class LMMSORTGenAIEvaluator(lmms):
    ...
    Implements the base lmms class interface for generate_until using ORT-GenAI's
    multimodal processor and generator. Image and audio inputs are rendered
    through the model's native structured-content chat template.
    """
    ...
    def loglikelihood(self, requests: list[Instance], disable_tqdm: bool = False) -> list[tuple[float, bool]]:
        """Reject loglikelihood tasks until upstream lmms-eval preserves tuple responses."""
        raise NotImplementedError("ortgenai_mm does not support lmms-eval loglikelihood tasks yet.")
