...
class LMMSEvaluator(OliveEvaluator):
    ...
    def __init__(self, tasks: list[str], **kwargs):
        ...
        # "lossless" preserves historical Olive runs; "jpeg85" forces JPEG.
    ...
    def _build_hf_lm(self, model: HfModelHandler, device: Device):
        ...
        if model.adapter_path:
            raise ValueError(
                "LMMSEvaluator does not support HfModelHandler.adapter_path because lmms-eval wrappers load the "
                "base checkpoint directly. Merge the adapter into a checkpoint before evaluation."
            )
        handler_load_kwargs = model.get_load_kwargs()
        if handler_load_kwargs:
            raise ValueError(
                "LMMSEvaluator does not forward HfModelHandler.load_kwargs because lmms-eval model wrappers have "
                "incompatible constructor options. Move wrapper-supported options to the evaluator's "
                f"hf_model_kwargs or remove the handler load_kwargs. Got keys: {sorted(handler_load_kwargs)}."
            )
    ...
    def evaluate(
        ...
        if tasks:
            ...
            # model wrapper (exposes generate_until over the resolved backend).
            if isinstance(model, ONNXModelHandler):
