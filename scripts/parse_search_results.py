...
def _fetch_model_sizes_from_blob(
    subscription_id: str,
    job_id: str,
    model_ids: set[str],
    ...
    # Data-plane access uses DefaultAzureCredential and the provided storage account URL/container.
    if not subscription_id:
        raise ValueError("subscription_id is required when resolving model sizes from blob storage")
    if not blob_account_url:
        raise ValueError("blob_account_url is required when resolving model sizes from blob storage")
...
def _enrich_records_with_model_sizes(
    records: list[dict[str, Any]],
    subscription_id: str,
    job_id: str,
    blob_account_url: str,
    ...
    try:
        sizes = _fetch_model_sizes_from_blob(subscription_id, job_id, model_ids, blob_account_url)
...
def process_log_with_blob_sizes(
    ...
    _enrich_records_with_model_sizes(records, subscription_id, job_id, blob_account_url)
    write_csv(records, search_cols, metric_cols, output_path)
    logger.info("Wrote %d rows to %s", len(records), output_path)
    return True
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Blob account URL for Olive outputs, e.g. https://<storage-account>.blob.core.windows.net",
    )
    args = parser.parse_args()

    if bool(args.subscription_id) != bool(args.job_id):
        parser.error("--subscription-id and --job-id must be provided together.")
    if bool(args.subscription_id) and not args.blob_account_url:
        parser.error("--blob-account-url is required when --subscription-id and --job-id are provided.")
    ...
    if args.input.is_dir():
        ...
        for log_file in log_files:
            ...
            if args.subscription_id and args.job_id:
                success = process_log_with_blob_sizes(
                    log_file,
                    output_csv,
