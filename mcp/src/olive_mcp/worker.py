...
def _handle_explore_passes(kwargs):
    ...
    for name, mc in sorted(olive_config.passes.items()):
        if provider:
            if "*" not in mc.supported_providers and provider not in mc.supported_providers:
                continue
        if precision:
            if "*" not in mc.supported_precisions and precision not in mc.supported_precisions:
                continue
        if accelerator:
            if "*" not in mc.supported_accelerators and accelerator not in mc.supported_accelerators:
                continue
        passes.append({
            "name": name,
